...
Vue.component('base-icon', require('./components/BaseIcon.vue').default);
Vue.use(Sortable);
...
function decryptMapApiKey(encryptedKey) {
    ...
    try {
        ...
        // Suppress console error as the key might just be unencrypted
        return null;
    }
}
...
Vue.prototype.$editorToolbar = "undo redo | blocks fontfamily fontsize | bold italic underline strikethrough | link image media table | align lineheight | numlist bullist indent outdent | charmap | code | removeformat";
Vue.prototype.$editorFont_size_formats = '8pt 10pt 12pt 14pt 16pt 18pt 24pt 36pt 48pt';

Vue.prototype.$tinymceImageUploadOptions = function () {
    ...
    return {
        ...
        images_upload_handler: function (blobInfo, progress) {
            return new Promise(function (resolve, reject) {
                ...
                window.axios.post(apiUrl + '/media/editor_upload', formData, {
                    ...
                    var msg = (err.response && err.response.data && err.response.data.error) || err.message || 'Upload failed';
                    reject(msg);
                });
            });
        },
        file_picker_callback: function (callback, value, meta) {
            ...
            input.onchange = function () {
                ...
                var fd = new FormData();
                fd.append('file', file);
                window.axios.post(apiUrl + '/media/editor_upload', fd)
                    .then(function (res) {
                        callback(res.data.location, { title: file.name });
                    })
...
window.trans = window.__ = function (string) {
    ...
    return _.get(lang, string) || string;
};
...
// Use translated message for HTML5 required-field validation (same pattern as show_a_lot: lang from localStorage).
...
// Make b-table empty text translatable (empty state and filtered-empty state).
// Override Bootstrap-Vue's b-table emptyText and emptyFilteredText defaults to use lang keys.
var BTableComponent = Vue.options.components['b-table'] || Vue.options.components['BTable'];
if (BTableComponent && BTableComponent.options && BTableComponent.options.props) {
    var props = BTableComponent.options.props;
    if (props.emptyText) {
        props.emptyText.default = function () {
            return (typeof window.__ === 'function') ? window.__('no_records_to_show') : 'There are no records to show';
        };
    }
    if (props.emptyFilteredText) {
        props.emptyFilteredText.default = function () {
            return (typeof window.__ === 'function') ? window.__('no_records_matching_your_request') : 'There are no records matching your request';
        };
    }
}
