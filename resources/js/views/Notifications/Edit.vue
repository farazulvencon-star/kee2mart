        ...
        <form ref="my-form" @submit.prevent="saveRecord">
            ...
            <div class="row">
                ...
                <div class="form-group" v-if="type === 'user'">
                    <label> {{ __('customer') }}<small>( Ex : 100,205, 360 comma separated )</small></label>
                    <i class="text-danger">*</i>
                    <Select2 v-model="type_ids" :placeholder="__('select_customer')" :options="users_options"
                        :settings="{ multiple: 'multiple', width: '100%', dropdownParent: '#mymodal' }" required />
                </div>
                <!--sellers-->
                <div class="form-group" v-if="type === 'seller'">
                    <label> {{ __('seller') }}<small>( Ex : 100,205, 360 comma separated )</small></label>
                    <i class="text-danger">*</i>
                    <Select2 v-model="type_ids" :placeholder="__('select_seller')" :options="sellers_options"
                        :settings="{ multiple: 'multiple', width: '100%', dropdownParent: '#mymodal' }" required />
                </div>
                <!--delivery boys-->
                <div class="form-group" v-if="type === 'delivery_boy'">
                    <label> {{ __('delivery_boys') }}<small>( Ex : 100,205, 360 comma separated )</small></label>
                    <i class="text-danger">*</i>
                    <Select2 v-model="type_ids" :placeholder="__('select_delivery_boy')"
                ...
                <div class="form-group">
                    ...
                    <div class="form-floating mb-3">
                        <textarea name="message" id="message" v-model="message" rows="40" cols="70" class="form-control"
                            :placeholder="__('enter_notification_message_here')"></textarea>
                    </div>
                </div>
                ...
                <div class="form-group" v-if="(include_image === true && id) || include_image === true">
                    ...
                    <input type="file" name="notification_image" accept="image/*" v-on:change="handleFileUpload"
...
export default {
    ...
    computed: {
        ...
        sellers_options: function () {
            var temp = [];
            this.sellers.forEach(seller => {
                temp.push({ id: seller.id, text: seller.name })
            });
            return temp;
        },
        ...
        delivery_boys_options: function () {
            ...
            this.delivery_boys.forEach(delivery_boy => {
                temp.push({ id: delivery_boy.id, text: delivery_boy.name })
            });
            return temp;
        },
