                ...
                <b-tab v-for="language in languages" :key="language.id">
                    ...
                    <div class="row">
                        ...
                        <template v-if="language.is_default">
                            ...
                            <div class="form-group col-md-6" v-if="globalColors && globalColors.country_background_color_mode === 'item-wise'">
                                <label>{{ __('background_color_for_light_theme') }}</label>
                                <div class="d-flex" style="gap: 10px; align-items: center; max-width: 500px;">
                                    <input type="text" class="form-control" v-model="background_color_for_light_theme">
                                    <input type="color" class="form-control cursor-pointer" style="max-width: 100px;" :value="(background_color_for_light_theme && background_color_for_light_theme.startsWith('#') && (background_color_for_light_theme.length === 4 || background_color_for_light_theme.length === 7)) ? background_color_for_light_theme : '#ffffff'" @input="background_color_for_light_theme = $event.target.value">
                                    <b-button size="sm" variant="outline-secondary" @click="background_color_for_light_theme = '#00000000'">
                                        {{ __('transparent') || 'Transparent' }}
                                    </b-button>
                                </div>
                                <small class="text-muted d-block mt-1">Note: Plain bg and bg less that's bg set perfectly category wise. That selected bg set in app and web cat image bg colur.</small>
                            </div>
                            <div class="form-group col-md-6" v-if="globalColors && globalColors.country_background_color_mode === 'item-wise'">
                                <label>{{ __('background_color_for_dark_theme') }}</label>
                                <div class="d-flex" style="gap: 10px; align-items: center; max-width: 500px;">
                                    <input type="text" class="form-control" v-model="background_color_for_dark_theme">
                                    <input type="color" class="form-control cursor-pointer" style="max-width: 100px;" :value="(background_color_for_dark_theme && background_color_for_dark_theme.startsWith('#') && (background_color_for_dark_theme.length === 4 || background_color_for_dark_theme.length === 7)) ? background_color_for_dark_theme : '#000000'" @input="background_color_for_dark_theme = $event.target.value">
                                    <b-button size="sm" variant="outline-secondary" @click="background_color_for_dark_theme = '#00000000'">
                                        {{ __('transparent') || 'Transparent' }}
                                    </b-button>
                                </div>
                                <small class="text-muted d-block mt-1">Note: Plain bg and bg less that's bg set perfectly category wise. That selected bg set in app and web cat image bg colur.</small>
...
export default {
    ...
    data() {
        return {
            ...
            logo: null,
            logoError: null,
            background_color_for_light_theme: this.record?.background_color_for_light_theme || this.globalColors?.country_background_color_light_theme || null,
            background_color_for_dark_theme: this.record?.background_color_for_dark_theme || this.globalColors?.country_background_color_dark_theme || null,
    ...
    methods: {
        ...
        resetForm() {
            ...
            this.logo_url = null
            this.status = 1
            this.background_color_for_light_theme = this.globalColors?.country_background_color_light_theme || null
            this.background_color_for_dark_theme = this.globalColors?.country_background_color_dark_theme || null
            this.translations = {}
        },
        ...
        loadCountryWithTranslations() {
            ...
            return axios.get(this.$apiUrl + '/countries', {
                ...
                .then(response => {
                    ...
                    this.background_color_for_light_theme = country.background_color_for_light_theme || this.globalColors?.country_background_color_light_theme || null;
                    this.background_color_for_dark_theme = country.background_color_for_dark_theme || this.globalColors?.country_background_color_dark_theme || null;
        ...
        async saveRecord() {
            ...
            try {
                ...
                if (this.background_color_for_light_theme) formData.append('background_color_for_light_theme', this.background_color_for_light_theme);
                if (this.background_color_for_dark_theme) formData.append('background_color_for_dark_theme', this.background_color_for_dark_theme);
