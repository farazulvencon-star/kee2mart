...
<head>
    ...
    $appTitle =
        is_array($appNameArr)
            ? ($appNameArr[$currentLang] ?? $appNameArr['en'] ?? 'eGrocer')
            : ($app_name ?: 'eGrocer');
@endphp
...
<body>
    ...
    <script src="{{ asset('assets/js/tinymce/code/plugin.min.js') }}"></script>
    ...
    <script>
        ...
        window.copyrightDetails = {!! json_encode($copyright_details ?? '') !!};
    ...
    <script>
        @if (
            ...
            // So login form can send fcm_token with login request; set when getToken() resolves.
