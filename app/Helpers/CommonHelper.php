...
class CommonHelper
{
    ...
    public static function getSettings($variables)
    {
        ...
        // Ensure all requested variables exist in the response, default to empty string if not set
    ...
    public static function getProductIdsSection($section, $user_id = null, $moduleId = null)
    {
        if (is_null($moduleId)) {
        ...
        if ($section->product_type == 'all_products') {
            ...
            if (!empty($user_id)) {
                $recentlyVisited = RecentlyVisitedProduct::join('products as p', 'recently_visited_products.product_id', '=', 'p.id')
                    ->where('recently_visited_products.user_id', $user_id)
                    ->where('p.module_id', $moduleId)
                    ->orderBy('recently_visited_products.visited_at', 'desc')
                    ->orderBy('recently_visited_products.created_at', 'desc')
                    ->limit(10)
                    ->pluck('recently_visited_products.product_id')
                    ->toArray();
    ...
    public static function getSectionWithProduct($seller_ids, $user_id = 0, $moduleId = 0)
    {
        $limit = 8;
        $offset =  0;
        ...
        foreach ($sections as $key => $section) {
            $product_ids = self::getProductIdsSection($section, $user_id, $moduleId) ?? "";
            ...
            foreach ($products as $row) {
                ...
                for ($k = 0; $k < count($variants); $k++) {
                    ...
                    if (isset($variant->discounted_price) && $variant->discounted_price > 0 && !empty($taxed->taxable_price) && $taxed->taxable_price > 0) {
                    ...
                    } elseif (isset($variant->discounted_price) && $variant->discounted_price > 0) {
    ...
    {
        $exists = OrderStatus::where('order_id', $order_status['order_id'])
            ->where('order_item_id', $order_status['order_item_id'] ?? 0)
    ...
    {
        $moduleId = request()->header('X-Module-Id') ?: 1;
        $period = request('orderStatusPeriod', 'yearly');

        $dateFilter = '';
        if ($period === 'monthly') {
            $dateFilter = ' AND orders.created_at >= "' . Carbon::now()->subDays(30)->toDateTimeString() . '"';
        } elseif ($period === 'weekly') {
            $dateFilter = ' AND orders.created_at >= "' . Carbon::now()->subDays(7)->toDateTimeString() . '"';
        } elseif ($period === 'yearly') {
            $dateFilter = ' AND orders.created_at >= "' . Carbon::now()->subDays(365)->toDateTimeString() . '"';
        }

        $query = '(select count(orders.id) from orders where orders.active_status = order_status_lists.id AND orders.deleted_at IS NULL AND orders.module_id = ' . intval($moduleId) . $dateFilter . ') AS order_count';

        if (isset($seller_id) && $seller_id != "" && $seller_id != 0) {
            $orderIds = OrderItem::where('seller_id', $seller_id)->get()->pluck('order_id')->toArray();
            if (count($orderIds) > 0) {
                $query = '(select count(orders.id) from orders where orders.active_status = order_status_lists.id AND orders.module_id = ' . intval($moduleId) . $dateFilter . ' and orders.id IN (' . implode(",", array_unique($orderIds)) . ')) AS order_count';
            }
        }
    ...
    public static function getOrderDetails($order_id, $exclude_cancelled_returned = false)
    {
        ...
        foreach ($order_items as $item) {
            ...
            $item->prescriptions = $prescriptionUrls;
        }
