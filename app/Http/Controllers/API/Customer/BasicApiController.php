...
class BasicApiController extends Controller
{
    ...
    public function getCategories(Request $request)
    {
        ...
        if ($is_own_data === 1) {
            ...
            $category = Category::where('status', 1)->where('module_id', $moduleId)->where('slug', $category_slug)->first();
            if (!$category) {
                return CommonHelper::responseError(__('no_category_found'));
            }
            $category->setRelation('catActiveChilds', collect());
            $category->makeHidden(['image']);
            $this->resolveItemColors($category, 'category');
            $response = [
                'status' => 1,
                'message' => __('success'),
                'total' => 1,
            ...
            $mode = Setting::get_value('category_background_color_mode') ?: 'global';
            if ($mode === 'global') {
                $lightColor = Setting::get_value('category_background_color_light_theme') ?? '';
                $darkColor = Setting::get_value('category_background_color_dark_theme') ?? '';
                $response['category_background_color_light_theme'] = $lightColor === 'transparent' ? '#00000000' : $lightColor;
                $response['category_background_color_dark_theme'] = $darkColor === 'transparent' ? '#00000000' : $darkColor;
            }
        ...
        if (isset($category_slug) && !empty($category_slug)) {
            ...
            $categories = Category::where('status', 1)->where('module_id', $moduleId)->where('parent_id', $category->id);
        } else {
            $categories = Category::where('status', 1)->where('module_id', $moduleId)->where('parent_id', $category_id);
        }
    ...
    private function resolveItemColors($item, $type)
    {
    ...
    public function getOffers(Request $request)
    {
        ...
        $offset = $request->offset ?? 0;
        $moduleId = request()->header('X-Module-Id') ?: 1;
        $baseQuery = Offer::where('module_id', $moduleId)->where(function ($q) {
    ...
    public function getSliders(Request $request)
    {
        ...
        $query = Slider::where('status', 1)->where('module_id', $moduleId);
        $total = $query->count();
    ...
    public function getSeller(Request $request)
    {
        ...
        $moduleId = request()->header('X-Module-Id') ?: 1;
        $seller = Seller::select('id', 'name', 'store_name', 'email', 'mobile', 'store_url', 'logo')
            ->where("id", $seller_id)
    ...
    public function getBrands(Request $request)
    {
        ...
        if ($brands->isNotEmpty()) {
            $brands->each(function ($brand) {
                $this->resolveItemColors($brand, 'brand');
            });
            $response = [
                'status' => 1,
                'message' => __('success'),
            ...
            $mode = Setting::get_value('brand_background_color_mode') ?: 'global';
            if ($mode === 'global') {
                $lightColor = Setting::get_value('brand_background_color_light_theme') ?? '';
                $darkColor = Setting::get_value('brand_background_color_dark_theme') ?? '';
                $response['brand_background_color_light_theme'] = $lightColor === 'transparent' ? '#00000000' : $lightColor;
                $response['brand_background_color_dark_theme'] = $darkColor === 'transparent' ? '#00000000' : $darkColor;
            }
    ...
    public function getCountries(Request $request)
    {
        ...
        if (!empty($countries)) {
            $countries->each(function ($country) {
                $this->resolveItemColors($country, 'country');
            });
            $response = [
                'status' => 1,
                'message' => __('success'),
            ...
            $mode = Setting::get_value('country_background_color_mode') ?: 'global';
            if ($mode === 'global') {
                $lightColor = Setting::get_value('country_background_color_light_theme') ?? '';
                $darkColor = Setting::get_value('country_background_color_dark_theme') ?? '';
                $response['country_background_color_light_theme'] = $lightColor === 'transparent' ? '#00000000' : $lightColor;
                $response['country_background_color_dark_theme'] = $darkColor === 'transparent' ? '#00000000' : $darkColor;
            }
