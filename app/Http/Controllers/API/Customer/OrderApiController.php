...
class OrderApiController extends Controller
{
    public function placeOrder(Request $request)
    {
        try {
            ...
            $moduleId = $request->header('X-Module-Id') ?? $request->get('module_id') ?? 1;

            $cartItems = Cart::select('carts.*', 'products.seller_id', 'products.name as product_name', 'products.require_prescription', 'sellers.name as seller_name', 'sellers.status as seller_status', 'sellers.self_pickup_mode', 'sellers.pickup_store_address', 'sellers.pickup_latitude', 'sellers.pickup_longitude', 'sellers.pickup_store_timings', 'sellers.mobile')
            ...
            $requires_prescription = $cartItems->contains('require_prescription', 1);
            $prescription_required_variant_ids = $cartItems->where('require_prescription', 1)->pluck('product_variant_id')->unique()->toArray();

            $groupedPrescriptions = [];
            if ($request->has('prescriptions') || $request->hasFile('prescriptions')) {
                $prescriptions = $request->input('prescriptions') ?? [];
                $prescriptionsFiles = $request->file('prescriptions') ?? [];
                $maxIndex = max(count($prescriptions), count($prescriptionsFiles));

                for ($i = 0; $i < $maxIndex; $i++) {
                    $variantIdsStr = $prescriptions[$i]['product_variant_ids'] ?? ($prescriptionsFiles[$i]['product_variant_ids'] ?? '');
                    $variantIdsStr = str_replace(['"', "'"], '', $variantIdsStr);
                    $files = $prescriptionsFiles[$i]['images'] ?? [];
                    if (!is_array($files)) {
                        $files = [$files];
                    }
                    $files = array_filter($files);

                    if ($variantIdsStr && !empty($files)) {
                        if (count($files) > 5) {
                            return CommonHelper::responseError(__('maximum_of_5_prescription_images_are_allowed_per_prescription_group'));
                        }

                        foreach ($files as $file) {
                            if (!($file instanceof \Symfony\Component\HttpFoundation\File\UploadedFile) || !$file->isValid()) {
                                return CommonHelper::responseError(__('invalid_prescription_file'));
                            }

                            $extension = strtolower($file->getClientOriginalExtension());
                            $allowedExtensions = ['jpeg', 'png', 'jpg', 'webp', 'pdf'];
                            if (!in_array($extension, $allowedExtensions)) {
                                return CommonHelper::responseError(__('prescription_file_must_be_an_image_or_pdf'));
                            }

                            $maxSize = 5 * 1024 * 1024; // 5MB
                            if ($file->getSize() > $maxSize) {
                                return CommonHelper::responseError(__('prescription_file_must_be_less_than_5mb'));
                            }
                        }

                        $variantIds = array_map('intval', explode(',', $variantIdsStr));
                        $groupedPrescriptions[] = [
            ...
            if ($requires_prescription) {
                if (!empty($groupedPrescriptions)) {
                    $coveredVariantIds = [];
                    foreach ($groupedPrescriptions as $gp) {
                        $coveredVariantIds = array_merge($coveredVariantIds, $gp['variant_ids']);
                    }
                    $coveredVariantIds = array_unique($coveredVariantIds);
                    $missingVariantIds = array_diff($prescription_required_variant_ids, $coveredVariantIds);
                    if (!empty($missingVariantIds)) {
                        return CommonHelper::responseError(__('prescription_is_required_for_one_or_more_items_in_your_cart'));
                    }
                } else {
                    return CommonHelper::responseError(__('prescription_is_required_for_one_or_more_items_in_your_cart'));
                }
            ...
            $items = str_replace(['"', "'"], '', $request->product_variant_id);
            ...
            try {
                ...
                foreach ($item_details as $key => $item) {
                    ...
                    $orderItemsMap[$product_variant_id] = $order_item->id;
                ...
                if (!empty($groupedPrescriptions)) {
                    foreach ($groupedPrescriptions as $gp) {
                        $groupImagePaths = [];
                        foreach ($gp['files'] as $file) {
                            $fileName = time() . '_' . rand(1111, 99999) . '.' . $file->getClientOriginalExtension();
                            $path = \Illuminate\Support\Facades\Storage::disk('public')->putFileAs('orders/prescriptions', $file, $fileName);
                            $groupImagePaths[] = $path;
                            $prescriptionImagesPath[] = $path;
                        }

                        foreach ($groupImagePaths as $imagePath) {
                            $orderPrescriptionId = DB::table('order_prescriptions')->insertGetId([
                                'order_id' => $order_id,
                                'image' => $imagePath,
                                'created_at' => now(),
                                'updated_at' => now()
                            ...
                            foreach ($gp['variant_ids'] as $vId) {
                                if (isset($orderItemsMap[$vId])) {
                                    DB::table('order_prescription_items')->insert([
                                        'order_prescription_id' => $orderPrescriptionId,
                                        'order_item_id' => $orderItemsMap[$vId],
                                        'created_at' => now(),
                                        'updated_at' => now()
                                    ]);
                                }
                    ...
                    if (!empty($prescriptionImagesPath)) {
                        $order->prescription_images = json_encode(array_values(array_unique($prescriptionImagesPath)));
                        $order->save();
                    }
                }
    ...
    public function getOrders(Request $request)
    {
        ...
        foreach ($res as $key => $row) {
            ...
            foreach ($items as $subkey => $item) {
                ...
                // Fetch prescriptions for this order item
                $itemPrescriptions = DB::table('order_prescription_items as opi')
                    ->join('order_prescriptions as op', 'opi.order_prescription_id', '=', 'op.id')
                    ->where('opi.order_item_id', $item->id)
                    ->select('op.image')
                    ->get();
                
                $prescriptionUrls = [];
                foreach ($itemPrescriptions as $ip) {
                    $prescriptionUrls[] = asset('storage/' . $ip->image);
                }
                $items[$subkey]->prescriptions = $prescriptionUrls;
            }
