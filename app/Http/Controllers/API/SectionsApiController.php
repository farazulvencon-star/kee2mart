...
class SectionsApiController extends Controller
{
    ...
    public function index()
    {
        ...
        $sections = $query->get()->map(function ($section) {
            ...
            $categoryIds = json_decode($section->category_ids, true) ?? [];
            $categoryNames = [];
    ...
    public function save(Request $request)
    {
        ...
        if ($request->has('product_ids') && is_array($request->product_ids) && count($request->product_ids) > 0) {
            $invalidProductsCount = \App\Models\Product::whereIn('id', $request->product_ids)
                ->where('module_id', '!=', $moduleId)
                ->count();
            if ($invalidProductsCount > 0) {
                return CommonHelper::responseError('Some selected products belong to a different module.');
            }
        }
        if ($request->has('category_ids') && is_array($request->category_ids) && count($request->category_ids) > 0) {
            $invalidCategoriesCount = \App\Models\Category::whereIn('id', $request->category_ids)
                ->where('module_id', '!=', $moduleId)
                ->count();
            if ($invalidCategoriesCount > 0) {
                return CommonHelper::responseError('Some selected categories belong to a different module.');
            }
        }
