...
class ProductApisController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        try {
            ...
            $product->indicator = (\App\Models\Product::getCurrentModuleId() == 2) ? 0 : $request->product_type;
            $product->manufacturer = $request->manufacturer;
            $product->made_in = $request->made_in;
            $product->tax_included_in_price = $request->tax_included_in_price;
            $product->return_status = $request->return_status;
            $product->require_prescription = $request->require_prescription ?? 0;
    ...
    public function update(Request $request)
    {
        ...
        try {
            ...
            $product->type = $request->type;
            $product->category_id = $request->category_id;
            $product->indicator = ($product->module_id == 2 || \App\Models\Product::getCurrentModuleId() == 2) ? 0 : $request->product_type;
            $product->manufacturer = $request->manufacturer;
            $product->made_in = $request->made_in;
            $product->tax_included_in_price = $request->tax_included_in_price;
            $product->return_status = $request->return_status;
            ...
            if ($_FILES["file"]["size"] > 0) {
                ...
                foreach ($dataRows as $products) {
                    if ($count != 0) {
                        ...
                        if (empty($products[1])) {
                            ...
                            if (auth()->user()->role_id == Role::$roleSeller && $products[1] != auth()->user()->seller->id) {
                                $rowErrors[] = 'Seller ID in the file (' . $products[1] . ') does not match your Seller ID (' . auth()->user()->seller->id . ').';
                            } else {
                ...
                foreach ($dataRows as $emapData) {
                    if ($count1 != 0) {
                        ...
                        $product->indicator = (\App\Models\Product::getCurrentModuleId() == 2) ? 0 : $indicator;
        ...
        try {
            ...
            if ($_FILES["file"]["size"] > 0) {
                ...
                foreach ($dataRows as $row) {
                    ...
                    // --- Validate Product ID existence and ownership ---
                    $productId = (int) ($rd['ID'] ?? 0);
                    $dbProduct = Product::find($productId);
                    if (!$dbProduct) {
                        $rowErrs[] = 'Product ID ' . $productId . ' does not exist';
                    } else {
                        if (auth()->user()->role_id == Role::$roleSeller && $dbProduct->seller_id != auth()->user()->seller->id) {
                            $rowErrs[] = 'Product ID ' . $productId . ' does not belong to you';
                        }
                    }
                    ...
                    if (empty(trim($rd['Product Name'] ?? ''))) {
                        ...
                        $nameExists = Product::where('name', trim($rd['Product Name']))->where('id', '!=', $productId)->exists();
                        if ($nameExists) {
                    ...
                    if (empty($rdSellerId) || !is_numeric($rdSellerId)) {
                        ...
                        if (auth()->user()->role_id == Role::$roleSeller && $rdSellerId != auth()->user()->seller->id) {
                            $rowErrs[] = 'Seller ID in the file (' . $rdSellerId . ') does not match your Seller ID (' . auth()->user()->seller->id . ').';
                        } else {
                ...
                foreach ($dataRows as $row) {
                    ...
                    $product = Product::updateOrCreate(
                        ...
                        [
                            ...
                            'indicator' => (\App\Models\Product::getCurrentModuleId() == 2) ? 0 : $indicator,
