...
class OrdersApiController extends Controller
{
    public function getOrders(Request $request)
    {
        ...
        $sellerIdSubquery = $sellerId
            ? DB::raw("(SELECT s.id FROM sellers s INNER JOIN order_items oi ON s.id = oi.seller_id WHERE oi.order_id = orders.id AND oi.seller_id = {$sellerId} LIMIT 1) as seller_id")
            : DB::raw('(SELECT s.id FROM sellers s INNER JOIN order_items oi ON s.id = oi.seller_id WHERE oi.order_id = orders.id LIMIT 1) as seller_id');
    ...
    public function getWeeklySales(Request $request)
    {
        ...
        if ($period === 'yearly') {
            $orders = Order::select(
                DB::raw('ROUND(SUM(remaining_final), 2) AS total_sale'),
                DB::raw('COUNT(id) AS total_orders'),
                DB::raw('MONTH(created_at) AS order_month')
            )
        ...
        if ($order) {
            ...
            try {
                dispatch(function () use ($order) {
                    ...
                    CommonHelper::sendOrderNotificationsToAdmins($order, 'order_assign', $order->delivery_boy_id);
                })->afterResponse();
            } catch (\Exception $e) {
