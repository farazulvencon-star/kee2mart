...
class CashfreeController extends Controller
{
        ...
        try {
            if ($notification['data']['order']['transaction_status'] ?? $notification['data']['payment']['payment_status']== 'SUCCESS') {
                ...
                if ($explode[0] == 'order') {
                     ...
                     $order = Order::withoutGlobalScope('module')->withTrashed()->where('id', $explode[1])->first();
                ...
                $explode = explode('-', $order_id);
                if ($explode[0] == 'order') {
                    Order::withoutGlobalScope('module')->where('id', $explode[1])->update(['active_status' => OrderStatusList::$cancelled]);
                }
            }
        } catch (\Exception $e) {
    ...
    if ($explode[0] == 'order') {
        ...
        $order_status = Order::withoutGlobalScope('module')->where('id', $explode[1])->where('user_id', $explode[2])->value('active_status');
